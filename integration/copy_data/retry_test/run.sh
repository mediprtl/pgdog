...
# Replication retry is fixed 500ms with unlimited attempts (see pgdog.toml), so CI shard
# restart latency cannot exhaust the budget. 60s covers copy of ~60k rows + replication catch-up.
...
# exhausted, non-retryable error) waiting for the canary would just time out at 60s with
# no useful signal. Check this BEFORE waiting for canary delivery to localise the failure.
if ! admin_psql -tAc 'SHOW TASKS' 2>/dev/null | grep -q '|replication|'; then
